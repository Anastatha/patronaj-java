...
public class UserResultSetMapper implements RowMapper<User> {
    ...
    public User mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        user.setCuratorId(rs.getLong("curator_id"));
        ...
        Long curatorEntityId = null;
        if (hasColumn(rs, "curator_entity_id")) {
            long tmp = rs.getLong("curator_entity_id");
            if (!rs.wasNull()) {
                curatorEntityId = tmp;
            }
        }

        if (curatorEntityId != null) {
            Curator curator = new Curator();
            curator.setId(curatorEntityId);
            ...
            curator.setPhone(getNullableString(rs, "curator_phone"));
            curator.setPatronymic(getNullableString(rs, "curator_patronymic"));
            String roleStr = getNullableString(rs, "curator_role");
            if (roleStr != null) {
